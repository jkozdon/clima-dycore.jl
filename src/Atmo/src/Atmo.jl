...
export atmo, defaultparameters, initialize
...
function defaultparameters()
  parameters = Dict()
  parameters[:γ] = 14  // 10
  parameters[:p0] = 100000
  parameters[:R_gas] = 28717 // 100
  parameters[:c_p] = 100467 // 100
  parameters[:c_v] = 7175 // 10
  parameters[:gravity] = 10
  parameters[:DFloat] = Float64
  parameters[:N] = 4
  parameters[:dim] = 3
...
function initialize(parameters)
  # local DFloat          = parameters[:DFloat]
  # local γ::DFloat       = parameters[:γ]
  # local p0::DFloat      = parameters[:p0]
  # local R_gas::DFloat   = parameters[:R_gas]
  # local c_p::DFloat     = parameters[:c_p]
  # local c_v::DFloat     = parameters[:c_v]
  # local gravity::DFloat = parameters[:gravity]
  # local N               = parameters[:N]
  # local dim             = parameters[:dim]
...
# {{{ atmo driver
function atmo(::Val{dim}, ::Val{N}, mpicomm, ic, mesh, tend;
